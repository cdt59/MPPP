...
class TestImageProcessing(unittest.TestCase):
    ...
    def test_ftau(self):
        self.assertAlmostEqual(self.processor._find_ftau(
            0.5, self.zcam_l.label, self.zcam_l.cam_type, self.zcam_l.filename), 0.42324086, 5)
        self.assertAlmostEqual(self.processor._find_ftau(
            0.5, self.zcam_r.label, self.zcam_r.cam_type, self.zcam_r.filename), 0.42324086, 5)
        self.assertAlmostEqual(self.processor._find_ftau(
            0.5, self.flf.label, self.flf.cam_type, self.flf.filename), 0.42324086, 5)
        self.assertAlmostEqual(self.processor._find_ftau(
            0.5, self.frf.label, self.frf.cam_type, self.frf.filename), 0.42324086, 5)
        self.assertAlmostEqual(self.processor._find_ftau(
            0.5, self.nlf.label, self.nlf.cam_type, self.nlf.filename), 0.42324086, 5)
        self.assertAlmostEqual(self.processor._find_ftau(
            0.5, self.nrf.label, self.nrf.cam_type, self.nrf.filename), 0.42324086, 5)
        self.assertAlmostEqual(
            self.processor._find_ftau(0.5, self.nlmv.label, self.nlmv.cam_type, self.nlmv.filename), 0.42324086, 5)
        self.assertAlmostEqual(
            self.processor._find_ftau(0.5, self.nrmv.label, self.nrmv.cam_type, self.nrmv.filename), 0.42324086, 5)

    def test_calculate_padding(self):
        self.assertEqual(self.processor._calculate_padding(self.zcam_l.image, self.zcam_l.label, self.zcam_l.cam_type, self.zcam_l.filename),
                         (0, 0, 0, 0))
        self.assertEqual(self.processor._calculate_padding(self.zcam_r.image, self.zcam_r.label, self.zcam_r.cam_type, self.zcam_r.filename),
                         (0, 0, 0, 0))
        self.assertEqual(self.processor._calculate_padding(self.flf.image, self.flf.label, self.flf.cam_type, self.flf.filename),
                         (0, 0, 0, 0))
        self.assertEqual(self.processor._calculate_padding(self.frf.image, self.frf.label, self.frf.cam_type, self.frf.filename),
                         (0, 0, 0, 0))
        self.assertEqual(self.processor._calculate_padding(self.nlf.image, self.nlf.label, self.nlf.cam_type, self.nlf.filename),
                         (1928, 1912, 1448, 1432))
        self.assertEqual(self.processor._calculate_padding(self.nrf.image, self.nrf.label, self.nrf.cam_type, self.nrf.filename),
                         (0, 0, 0, 0))
        self.assertEqual(self.processor._calculate_padding(self.nlmv.image, self.nlmv.label, self.nlmv.cam_type, self.nlmv.filename),
                         (0, 0, 0, 0))
        self.assertEqual(self.processor._calculate_padding(self.nrmv.image, self.nrmv.label, self.nrmv.cam_type, self.nrmv.filename),
                         (0, 0, 0, 0))

    def test_make_image_mask(self):
        self.zcam_l.image = self.processor._single_to_triple_channels(
            self.zcam_l.image, self.zcam_l.cam_type)
        zcam_l_padded_image = self.processor._pad_image(
            self.zcam_l.image, (0, 0, 0, 0))
        self.assertEqual(self.processor._make_image_mask(self.zcam_l.image, (0, 0, 0, 0), self.zcam_l.pad_im, self.zcam_l.filename).shape,
                         zcam_l_padded_image.shape[:2])

        self.zcam_r.image = self.processor._single_to_triple_channels(
            self.zcam_r.image, self.zcam_r.cam_type)
        zcam_r_padded_image = self.processor._pad_image(
            self.zcam_r.image, (0, 0, 0, 0))
        self.assertEqual(self.processor._make_image_mask(self.zcam_r.image, (0, 0, 0, 0), self.zcam_r.pad_im, self.zcam_r.filename).shape,
                         zcam_r_padded_image.shape[:2])

        self.flf.image = self.processor._single_to_triple_channels(
            self.flf.image, self.flf.cam_type)
        flf_padded_image = self.processor._pad_image(
            self.flf.image, (0, 0, 0, 0))
        self.assertEqual(self.processor._make_image_mask(self.flf.image, (0, 0, 0, 0), self.flf.pad_im, self.flf.filename).shape,
                         flf_padded_image.shape[:2])

        self.frf.image = self.processor._single_to_triple_channels(
            self.frf.image, self.frf.cam_type)
        frf_padded_image = self.processor._pad_image(
            self.frf.image, (0, 0, 0, 0))
        self.assertEqual(self.processor._make_image_mask(self.frf.image, (0, 0, 0, 0), self.frf.pad_im, self.frf.filename).shape,
                         frf_padded_image.shape[:2])

        self.nlf.image = self.processor._single_to_triple_channels(
            self.nlf.image, self.nlf.cam_type)
        nlf_padded_image = self.processor._pad_image(
            self.nlf.image, (1928, 1912, 1448, 1432))
        self.assertEqual(self.processor._make_image_mask(self.nlf.image, (1928, 1912, 1448, 1432),
                         self.nlf.pad_im, self.nlf.filename).shape, nlf_padded_image.shape[:2])

        self.nrf.image = self.processor._single_to_triple_channels(
            self.nrf.image, self.nrf.cam_type)
        nrf_padded_image = self.processor._pad_image(
            self.nrf.image, (0, 0, 0, 0))
        self.assertEqual(self.processor._make_image_mask(self.nrf.image, (0, 0, 0, 0), self.nrf.pad_im, self.nrf.filename).shape,
                         nrf_padded_image.shape[:2])

        self.nlmv.image = self.processor._single_to_triple_channels(
            self.nlmv.image, self.nlmv.cam_type)
        nlmv_padded_image = self.processor._pad_image(
            self.nlmv.image, (0, 0, 0, 0))
        self.assertEqual(self.processor._make_image_mask(self.nlmv.image, (0, 0, 0, 0),
                                                         self.nlmv.pad_im, self.nlmv.filename).shape,
                         nlmv_padded_image.shape[:2])

        self.nrmv.image = self.processor._single_to_triple_channels(
            self.nrmv.image, self.nrmv.cam_type)
        nrmv_padded_image = self.processor._pad_image(
            self.nrmv.image, (0, 0, 0, 0))
        self.assertEqual(self.processor._make_image_mask(self.nrmv.image, (0, 0, 0, 0), self.nrmv.pad_im, self.nrmv.filename).shape,
