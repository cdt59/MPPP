...
class image:
    ...
    def __init__(self, IMG_path, just_label=False, frame='site3'):
        ...
        if not just_label:
            # three band float-32 image array
            self.image = np.float32(PDS3Image.open(IMG_path).image)
            # one band boolian image array
            self.mask_image = np.ones(self.image.shape[:2])*255
    ...
    def image_process(self):
        ...
        if self.filename.split('_N')[0][-3:] == 'RZS':
            self.ftau = np.float32(
                self.label['DERIVED_IMAGE_PARMS']['RAD_ZENITH_SCALING_FACTOR'])
        ...
        if len(self.image.shape) == 2:
            if 'MV' in self.filename or 'M_' in self.filename:
                ...
                self.image = colour_demosaicing.demosaicing_CFA_Bayer_Malvar2004(
                    self.image, 'RGGB')

        d = 57.296
        try:
            self.mu = np.sin(
                self.label['SITE_DERIVED_GEOMETRY_PARMS']['SOLAR_ELEVATION'][0]/d)
        except:
            self.mu = 1.0
        ...
        self.ftau_min = 0.2
        if self.ftau < self.ftau_min:
            self.ftau = self.ftau_min
        ...
        if self.filename[0] == 'Z':
            ...
            if self.pad_im:
                if self.im.shape != (self.full_height, self.full_width, 3):
                    ...
                    self.pad_right = self.full_width - \
                        self.label['MINI_HEADER']['LINE_SAMPLES'] - \
                        self.label['MINI_HEADER']['FIRST_LINE_SAMPLE'] + 1
                    self.pad_top = self.label['MINI_HEADER']['FIRST_LINE'] - 1
                    self.pad_bottom = self.full_height - \
                        self.label['MINI_HEADER']['LINES'] - \
                        self.label['MINI_HEADER']['FIRST_LINE'] + 1
                    ...
                    if self.pad_top != 0 or self.pad_bottom != 0 or self.pad_left != 0 or self.pad_right != 0:
                        ...
                        self.im = pad_image(
                            self.image, pad=[self.pad_left, self.pad_right, self.pad_top, self.pad_bottom])
            ...
            if self.pad_im:
                if (self.down_sample == '0' and self.im.shape != (3840, 5120, 3)) or \
                    ...
                    if self.down_sample == '0':
                        self.full_height, self.full_width = [3840, 5120]
                    if self.down_sample == '1':
                        self.full_height, self.full_width = [1920, 2560]
                    if self.down_sample == '2':
                        self.full_height, self.full_width = [960, 1280]

                    self.pad_left = np.min(
                        self.label['INSTRUMENT_STATE_PARMS']['TILE_FIRST_LINE_SAMPLE']) - 1
                    self.pad_right = self.full_width - \
                        np.max(self.label['INSTRUMENT_STATE_PARMS']
                               ['TILE_FIRST_LINE_SAMPLE']) - 1280 + 1
                    self.pad_top = np.min(
                        self.label['INSTRUMENT_STATE_PARMS']['TILE_FIRST_LINE']) - 1
                    self.pad_bottom = self.full_height - \
                        np.max(self.label['INSTRUMENT_STATE_PARMS']
                               ['TILE_FIRST_LINE']) - 960 + 1
                    ...
                    if self.pad_top != 0 or self.pad_bottom != 0 or self.pad_left != 0 or self.pad_right != 0:
                        ...
                        self.im = pad_image(
                            self.im, pad=[self.pad_left, self.pad_right, self.pad_top, self.pad_bottom])
        ...
        if (self.pad_top != 0 or self.pad_bottom != 0 or self.pad_left != 0 or self.pad_right != 0) and self.pad_im:
            ...
            self.mask_im = pad_image(self.mask_im, pad=[
                                     self.pad_left, self.pad_right, self.pad_top, self.pad_bottom])

            if self.pad_bottom == 0 and self.pad_right == 0:
                self.mask_im[self.pad_top:,
                             self.pad_left:][self.image[:, :, 1] == 0] = 0
            elif self.pad_bottom == 0:
                self.mask_im[self.pad_top:,                 self.pad_left:-
                             self.pad_right][self.image[:, :, 1] == 0] = 0
            elif self.pad_right == 0:
                self.mask_im[self.pad_top:-self.pad_bottom,
                             self.pad_left:][self.image[:, :, 1] == 0] = 0
            else:
                self.mask_im[self.pad_top:-self.pad_bottom, self.pad_left:-
                             self.pad_right][self.image[:, :, 1] == 0] = 0
        else:
            ...
            if self.filename[0] == 'F':
                ...
                if self.filename[:2] == 'FL':
                    mask_path = os.path.join(
                        parent_path, 'params/FL{}.jpg'.format(self.down_sample))
                else:
                    mask_path = os.path.join(
                        parent_path, 'params/FR{}.jpg'.format(self.down_sample))
...
#             self.tau = 0.5
    ...
    def image_reference(self):
        ...
        if 0:
            ...
            parent_path = os.path.split(os.getcwd())[0]
            df_cmods_v2_path = os.path.join(
                parent_path, 'params/Tables - CAHV vs focus.csv')
            ...
            if 1:
                ...
                string = ' MODEL_COMPONENT_1 = ({:5.8f},{:5.8f},{:5.8f})\n'.format(cmod_est_['MODEL_COMPONENT_1'][0], cmod_est_['MODEL_COMPONENT_1'][1], cmod_est_['MODEL_COMPONENT_1'][2], ) \
                    ...
                    + ' MODEL_COMPONENT_6 = ({:5.8f},{:5.8f},{:5.8f})\n'.format(cmod_est_[
                        'MODEL_COMPONENT_6'][0], cmod_est_['MODEL_COMPONENT_6'][1], cmod_est_['MODEL_COMPONENT_6'][2], )
    ...
    def cmod_from_cahvor(self, GEOMETRIC_CAMERA_MODEL):
        ...
        self.C = np.array(
            GEOMETRIC_CAMERA_MODEL['MODEL_COMPONENT_1'], dtype=np.float64)
        self.A = np.array(
            GEOMETRIC_CAMERA_MODEL['MODEL_COMPONENT_2'], dtype=np.float64)
        self.H = np.array(
            GEOMETRIC_CAMERA_MODEL['MODEL_COMPONENT_3'], dtype=np.float64)
        self.V = np.array(
            GEOMETRIC_CAMERA_MODEL['MODEL_COMPONENT_4'], dtype=np.float64)
        self.O = np.array(
            GEOMETRIC_CAMERA_MODEL['MODEL_COMPONENT_5'], dtype=np.float64)
        self.R = np.array(
            GEOMETRIC_CAMERA_MODEL['MODEL_COMPONENT_6'], dtype=np.float64)
        ...
        self.w, self.h = [self.label['IMAGE']
                          ['LINE_SAMPLES'], self.label['IMAGE']['LINES']]
        if self.w == 1600:
            self.w = 1648
    ...
    def find_Rt_veh2site_inginuity(self):
        ...
        self.az = 0
        self.az_veh = 0
        self.C = np.array(self.label['GEOMETRIC_CAMERA_MODEL']['MODEL_COMPONENT_1']) - np.array(
            self.label['GEOMETRIC_CAMERA_MODEL']['MODEL_TRANSFORM_VECTOR'])

        self.q_HELI_M = q_wxyz2xyzw(
            self.label['HELI_M_COORDINATE_SYSTEM']['ORIGIN_ROTATION_QUATERNION'])
        self.R_HELI_M = R.from_quat(self.q_HELI_M)
        self.C_HELI_M = self.R_HELI_M.apply(self.C, inverse=0) \
            + np.array(self.label['HELI_M_COORDINATE_SYSTEM']
                       ['ORIGIN_OFFSET_VECTOR'])

        self.q_HELI_M = q_wxyz2xyzw(
            self.label['HELI_S2_COORDINATE_SYSTEM']['ORIGIN_ROTATION_QUATERNION'])
        self.R_HELI_M = R.from_quat(self.q_HELI_M)
        self.C_HELI_M = self.R_HELI_M.apply(self.C, inverse=0) \
            + np.array(self.label['HELI_S2_COORDINATE_SYSTEM']
                       ['ORIGIN_OFFSET_VECTOR'])

        self.q_HELI_G = q_wxyz2xyzw(
            self.label['HELI_G_COORDINATE_SYSTEM']['ORIGIN_ROTATION_QUATERNION'])
        self.R_HELI_G = R.from_quat(self.q_HELI_G)
        self.C_HELI_G = self.R_HELI_G.apply(self.C_HELI_M, inverse=0) \
            + np.array(self.label['HELI_G_COORDINATE_SYSTEM']
                       ['ORIGIN_OFFSET_VECTOR'])

        R_HELI_takeoff = self.R_HELI_G.inv() * self.R_HELI_M
        C_HELI_takeoff = R_HELI_takeoff.apply(
            self.label['HELI_G_COORDINATE_SYSTEM']['ORIGIN_OFFSET_VECTOR'])
...
#         print( self.X_shift, self.Y_shift, self.Z_shift )
    ...
    def find_Rt_veh2site_perseverance(self):
        ...
        try:
            self.pan = np.rad2deg(
                self.label['RSM_ARTICULATION_STATE']['ARTICULATION_DEVICE_ANGLE'][0])
            self.tilt = np.rad2deg(
                self.label['RSM_ARTICULATION_STATE']['ARTICULATION_DEVICE_ANGLE'][1])
        except:
            self.pan = np.rad2deg(
                self.label['RSM_ARTICULATION_STATE']['ARTICULATION_DEVICE_ANGLE'][0][0])
            self.tilt = np.rad2deg(
                self.label['RSM_ARTICULATION_STATE']['ARTICULATION_DEVICE_ANGLE'][1][0])
        ...
        if self.frame == 'site3':
            ...
            self.q = q_wxyz2xyzw(
                self.label['ROVER_COORDINATE_SYSTEM']['ORIGIN_ROTATION_QUATERNION'])

            self.R_veh = R.from_quat(self.q)
            self.Cr = self.R_veh.apply(self.C, inverse=0)

            self.xyz_veh = np.array(
                self.label['ROVER_COORDINATE_SYSTEM']['ORIGIN_OFFSET_VECTOR'])
            self.xyz = self.Cr + \
                np.array(self.label['ROVER_COORDINATE_SYSTEM']
                         ['ORIGIN_OFFSET_VECTOR'])
        ...
        if not self.find_offsets_mode:
            ...
            if self.frame == 'site3':
                self.X_shift, self.Y_shift, self.Z_shift = XYZ_shift_offsets(
                    self.site, self.drive)
            else:
                self.X_shift, self.Y_shift, self.Z_shift = [0, 0, 0]
...
def pad_image(im, pad=[0, 0, 0, 0]):
    ...
    if len(im.shape) == 3:
        im = np.hstack([np.zeros((im.shape[0],   pad[0], 3)),
                       im, np.zeros((im.shape[0],  pad[1], 3)), ])
        im = np.vstack([np.zeros((pad[2], im.shape[1],  3)),
                       im, np.zeros((pad[3],  im.shape[1], 3)), ])
    else:
        im = np.hstack([np.zeros((im.shape[0],   pad[0])), im,
                       np.zeros((im.shape[0],  pad[1])), ])
        im = np.vstack([np.zeros((pad[2], im.shape[1],)), im,
                       np.zeros((pad[3],  im.shape[1])), ])
    return im
...
def XYZ_shift_offsets(site, drive):
    ...
    parent_path = os.path.split(os.getcwd())[0]
    waypoint_shift_path = os.path.join(
        parent_path, 'params/Mars2020_waypoint_shifts.csv')
    ...
    if drive in site_drives:
        drive_site_shift = site_shifts[np.where(
            site_shifts[:, 2] == drive)[0]][0, :]

    elif drive > site_drives.min() and drive < site_drives.max():
        drive_site_shift = interp1d(
            site_shifts[:, 2], site_shifts, axis=0)(drive)
...
def remove_duplicate_IMGs(IMG_paths):
    ...
    duplicates = list(
        set([names[i] for i, x in enumerate(names) if i != names.index(x)]))
...
#     find_offsets_mode = 1
    ...
    scale_z,  scale_red_z,  scale_blue_z = [
        1.0*scale_scale, 0.7, 1.5]  # Mastcam-Z
    scale_l,  scale_red_l,  scale_blue_l = [
        1.0*scale_scale, 0.75, 1.40]  # SuperCam RMI
    scale_s,  scale_red_s,  scale_blue_s = [
        1.0*scale_scale, 0.85, 1.40]  # SHERLOC WATSON

    # color balance parameters for the Mars 2020 engineering cameras
    scale_n,  scale_red_n,  scale_blue_n = [
        1.1*scale_scale, 0.75, 1.2]  # Navcam
    scale_v,  scale_red_v,  scale_blue_v = [
        1.5*scale_scale, 1.08, 0.96]  # Grayscale VCE Navcam
    scale_f,  scale_red_f,  scale_blue_f = [
        1.1*scale_scale, 0.78, 1.25]  # Front Hazcam
    scale_r,  scale_red_r,  scale_blue_r = [
        1.1*scale_scale, 0.78, 1.25]  # Rear Hazcam
    scale_hr, scale_red_hr, scale_blue_hr = [
        1.0*scale_scale, 0.75, 1.43]  # Inginuity RTE
    scale_hn, scale_red_hn, scale_blue_hn = [
        1.0*scale_scale, 1.08, 0.92]  # Inginuity Navcam
    scale_hn, scale_red_hn, scale_blue_hn = [
        1.2*scale_scale, 1.08, 0.92]  # Inginuity Navcam
...
#                         print( im.focus_mc, D_mc , file_extension )
            ...
            im.save_path_full = make_save_path(
                im.IMG_path, directory_output, fullpath=True, file_extension=file_extension)
            im.save_path = make_save_path(
                im.IMG_path, directory_output, fullpath=False)
...
#                  +str( np.round( im.roll, 4))+'\n'
                ...
                print('XYZ_ENU = [{:0.3f}, {:0.3f}, {:0.3f}] YPR = [{:0.2f}, {:0.2f}, {:0.2f}]  OPK = [{:0.2f}, {:0.2f}, {:0.2f}]'.format(
                    im.X, im.Y, im.Z, im.yaw, im.pitch, im.roll, im.omega, im.phi, im.kappa))
                print()
...
#     print( 'saved', csv_save_path )
    ...
    csv_save_path = os.path.dirname(
        csv_save_path)+'/positions_'+suf+'_'+frame+'_'+current_time + '.txt'
    with open(csv_save_path, 'w') as file:
    ...
    if find_offsets_mode:
        ...
        np.savetxt(directory_output+"/offsets_" +
...
def cahvor_pixels_2_ray(GEOMETRIC_CAMERA_MODEL, xy_pixels, rotate_2_camera_frame=False):
    ...
    zeta = np.tensordot(
        ray_cahv_R[:, :, np.newaxis, :], cahvor_R[4].reshape(1, 3))
    ...
    ray_cahv_R_prime_norm = ray_cahv_R_prime / \
        norm(ray_cahv_R_prime, axis=-1)[:, :, np.newaxis]
...
def cmod_transform(cmod, inverse=1):
    ...
    if inverse == 1:
        cmodr['MODEL_COMPONENT_1'] = Rot.apply(np.array(
            cmod['MODEL_COMPONENT_1']) - np.array(cmod['MODEL_TRANSFORM_VECTOR']), inverse=inverse)
    else:
        # cmodr['MODEL_COMPONENT_1']  = Rot.apply( np.array(cmod['MODEL_COMPONENT_1'])- np.array(cmod['MODEL_TRANSFORM_VECTOR']), inverse=inverse )
        cmodr['MODEL_COMPONENT_1'] = Rot.apply(np.array(
            cmod['MODEL_COMPONENT_1']), inverse=inverse) + np.array(cmod['MODEL_TRANSFORM_VECTOR'])
        # cmodr['MODEL_COMPONENT_1']  = np.array(cmod['MODEL_COMPONENT_1']) + Rot.apply( - np.array(cmod['MODEL_TRANSFORM_VECTOR']), inverse=inverse )

    cmodr['MODEL_COMPONENT_2'] = Rot.apply(
        cmod['MODEL_COMPONENT_2'], inverse=inverse)
    cmodr['MODEL_COMPONENT_3'] = Rot.apply(
        cmod['MODEL_COMPONENT_3'], inverse=inverse)
    cmodr['MODEL_COMPONENT_4'] = Rot.apply(
        cmod['MODEL_COMPONENT_4'], inverse=inverse)
    cmodr['MODEL_COMPONENT_5'] = Rot.apply(
        cmod['MODEL_COMPONENT_5'], inverse=inverse)
...
def find_M_ECC(im1, im2, plot=0):
    ...
    criteria = (cv2.TERM_CRITERIA_EPS |
                cv2.TERM_CRITERIA_COUNT, iterations, threshold)
    ...
    retval, m = cv2.findTransformECC(
        im1, im2, m, cv2.MOTION_AFFINE, criteria, mask, blur)
    ...
    print('quality of adjustment', np.round(retval, 5), ' scale change {:04.2f}%'.format(
        100-100*sp), 'center motion', np.round(np.array(dp)[0, :], 5), 'pixels')
...
def T_RM_from_cmod(cmod):
    ...
    R_RM = R.from_quat(q_wxyz2xyzw(
        cmod['MODEL_TRANSFORM_QUATERNION'])).as_matrix()
...
def T_CM_from_saved(cam):
    ...
    tL026_CM_i0, tR026_CM_i0, tL034_CM_i0, tR034_CM_i0, tL048_CM_i0, tR048_CM_i0, tL063_CM_i0, tR063_CM_i0, tL079_CM_i0, tR079_CM_i0, tL100_CM_i0, tR100_CM_i0, tL110_CM_i0, tR110_CM_i0 = \
        np.array([0.01, 0.01, 0.00, 0.00, -0.02, -0.02, -0.03, -
                 0.03, -0.025, -0.025, -0.025, -0.025, -0.02, -0.02])
    ...
    if cam[:2] == 'ZL':
        ...
        if cam[2:] == '026':
            t_CM += np.array([tL026_CM_i0, 0, 0])
        if cam[2:] == '034':
            t_CM += np.array([tL034_CM_i0, 0, 0])
        if cam[2:] == '048':
            t_CM += np.array([tL048_CM_i0, 0, 0])
        if cam[2:] == '063':
            t_CM += np.array([tL063_CM_i0, 0, 0])
        if cam[2:] == '079':
            t_CM += np.array([tL079_CM_i0, 0, 0])
        if cam[2:] == '100':
            t_CM += np.array([tL100_CM_i0, 0, 0])
        if cam[2:] == '110':
            t_CM += np.array([tL110_CM_i0, 0, 0])
    ...
    if cam[:2] == 'ZR':
        ...
        if cam[2:] == '026':
            t_CM += np.array([tR026_CM_i0, 0, 0])
        if cam[2:] == '034':
            t_CM += np.array([tR034_CM_i0, 0, 0])
        if cam[2:] == '048':
            t_CM += np.array([tR048_CM_i0, 0, 0])
        if cam[2:] == '063':
            t_CM += np.array([tR063_CM_i0, 0, 0])
        if cam[2:] == '079':
            t_CM += np.array([tR079_CM_i0, 0, 0])
        if cam[2:] == '100':
            t_CM += np.array([tR100_CM_i0, 0, 0])
        if cam[2:] == '110':
            t_CM += np.array([tR110_CM_i0, 0, 0])
...
def CAHVOR_M_model(z, fl, f):
    ...
    if z == 0 and fl == 34:
        ...
        b1_est, b2_est, cx_est, cy_est, k1_est, k2_est = [
            0.,    0.,  862.73050974,  618.07206458, -0.45922782,    0.54560116]
    ...
    if z == 1 and fl == 34:
        ...
        b1_est, b2_est, cx_est, cy_est, k1_est, k2_est = [
            0.,    0.,  846.77240069,  645.3408401, -0.45143535,    0.33797551]
        cx0, cx1, cy0, cy1 = [cx_est, 0, cy_est, 0]
